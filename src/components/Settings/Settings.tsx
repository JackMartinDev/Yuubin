...
const Settings = ({ closeModal }: Props) => {
  const { language, theme, dataPath, preserveOpenTabs } = useSelector(
  ...
  const selectedOption = languages.find(
    (item) => item.value === form.getValues().language,
  );
  ...
  const onSubmitHandler = (values: typeof form.values) => {
    ...
    console.log(config);
    invoke<TauriResponse>("edit_config", {
      data: JSON.stringify(snakecaseKeys(config)),
    })
      .then((response) => {
        if (response.success) {
          dispatch(
  ...
  return (
    <form onSubmit={form.onSubmit((values) => onSubmitHandler(values))}>
      ...
      <Group align="end" gap={4}>
        ...
        />
        <Button onClick={selectDirectory}>{t("browse")}</Button>
      </Group>
      <Divider mt="lg" />
