...
const Collection = ({ collection }: Props): JSX.Element => {
    ...
    invoke<TauriResponse>("delete_directory", { collection: collection.name })
      .then((response) => {
        if (response.success) {
          ...
          const newTabs = activeRequests.filter(
            (id) => !collectionIds.includes(id),
          );
          const newCollections = files.filter(
  ...
  const addRequestHandler = async (values: typeof form.values) => {
    ...
    invoke<TauriResponse>("create_file", {
      ...
      >
        <Flex justify="left" align="center" ml="xs">
          <IconChevronRight
            size={16}
            stroke={2}
            className={cx(classes.icon, { [classes.toggle]: isToggled })}
          />
