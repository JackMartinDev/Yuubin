...
function App(): JSX.Element {
  ...
  const activeTab = useSelector(
    (state: RootState) => state.request.activeRequest,
  );
  const files = useSelector((state: RootState) => state.request.files);
  const activeRequests = useSelector(
    (state: RootState) => state.request.activeRequests,
  );

  const syncFileSystem = () => {
    //Consider having these 2 invokes as a single "sync" invoke
    invoke<TauriResponse>("sync_files")
      .then((response) => dispatch(updatefiles(JSON.parse(response.message))))
      .catch((error) => {
        notifications.show({
    ...
    invoke<TauriResponse>("sync_config")
      .then((response) => {
        const { theme, dataPath, language, preserveOpenTabs }: Config =
          camelcaseKeys(JSON.parse(response.message));
        i18next.changeLanguage(language);
        setColorScheme(theme);
        dispatch(
          updateSettings({ theme, dataPath, language, preserveOpenTabs }),
        );
        if (preserveOpenTabs) {
    ...
    <Box h="100vh">
      ...
      <PanelGroup direction="horizontal">
        <Panel
          ...
          className={
            colorScheme === "dark"
              ? classes.fileTreeDark
              : classes.fileTreeLight
          }
        >
                  ...
                  files.flatMap((collection) =>
                    collection.requests
                      ...
                      .map((request) => (
                        <Tabs.Tab
                          key={request.meta.id}
                          value={request.meta.id}
                          p="xs"
                        >
                          <Flex align="center" gap="4px" maw={175}>
                            <Text className={classes.truncate} size="sm">
                              <MethodIcon method={request.method} />{" "}
                              {request.meta.name}
                            </Text>
                            <CloseButton
                              onClick={(event) =>
                                onCloseHandler(event, request.meta.id)
                              }
                              size="sm"
                            />
                          </Flex>
