...
const Request = ({ request, collectionName }: Props) => {
  ...
  const deleteHandler = () => {
    ...
    invoke<TauriResponse>("delete_file", {
        ...
        if (response.success) {
          ...
          const newCollections = files.map((collection) => {
            ...
            const filteredRequests = collection.requests.filter(
              (req) => req.meta.id !== request.meta.id,
            );
  ...
  return (
    ...
    >
      <Tooltip
        label={request.meta.name}
        openDelay={300}
        position="right"
        offset={{ mainAxis: -20, crossAxis: -35 }}
      >
        <Text className={classes.truncate} size="sm" ml="xs">
          <MethodIcon method={request.method} /> {request.meta.name}
        </Text>
      </Tooltip>
      <Menu shadow="md" width={200}>
        <Menu.Target>
          <ActionIcon
            ...
            color="dark"
            style={
              hovered ? { visibility: "visible" } : { visibility: "hidden" }
            }
          >
        ...
        <Menu.Dropdown>
          ...
          <Menu.Item
            leftSection={
              <IconBallpen style={{ width: rem(16), height: rem(16) }} />
            }
            onClick={(event) => {
              renameHandler(event);
            }}
